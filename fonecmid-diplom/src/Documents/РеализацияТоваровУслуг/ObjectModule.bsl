...
Процедура ВКМ_ВыполнитьАвтозаполнение()Экспорт	
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	BKM_НоменклатураАбонентскаяПлата.Значение КАК НоменклатураАбонентскаяПлата,
		|	BKM_НоменклатураРаботыСпециалиста.Значение КАК НоменклатураРаботыСпециалиста
		|ИЗ
...
Процедура ЗаполнитьТЧУслуги(Договор, Контрагент, Дата, НоменклатураАбонентскаяПлата, НоменклатураРаботыСпециалиста)
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ЕСТЬNULL(ДоговорыКонтрагентов.BKM_СуммаАбонентскойПлаты, 0) КАК СуммаАбонентскойПлаты
		|ИЗ
		...
		|ИЗ
		|	РегистрНакопления.BKM_ВыполненныеКлиентуРаботы.Обороты(&НачалоМесяца, &КонецМесяца,, Клиент = &Контрагент
	...
	Если ВыборкаВыполненныеРаботы.Следующий() Тогда
		ВыполненныеРаботыЧас = ВыборкаВыполненныеРаботы.КоличествоЧасовОборот;
		ВыполненныеРаботыСумма = ВыборкаВыполненныеРаботы.СуммаКОплатеОборот;
		НоваяСтрока = Услуги.Добавить();
