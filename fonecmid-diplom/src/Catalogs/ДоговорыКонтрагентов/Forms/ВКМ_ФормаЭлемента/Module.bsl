...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи) 
	...
	Иначе
		...
		ТекущийОбъект.BKM_СуммаАбонентскойПлаты = 0;
		ТекущийОбъект.BKM_СтоимостьЧаса = 0;
...
Процедура ДобавитьНовыеЭлементыФормы()	
	...
	Если Элементы.Найти("СуммаАбонентскойПлаты") = Неопределено Тогда
		НовыйЭлемент = Элементы.Добавить("АСуммаАбонентскойПлаты", Тип("ПолеФормы"),ГруппаПрограммноСозданныеЭлементыФормы);		
		НовыйЭлемент.ПутьКДанным = "Объект.BKM_СуммаАбонентскойПлаты";
		НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
		НовыйЭлемент.Заголовок = "Ежемесячная абонентская плата"; 
	КонецЕсли;
	
	Если Элементы.Найти("СтоимостьЧаса") = Неопределено Тогда
		НовыйЭлемент = Элементы.Добавить("СтоимостьЧаса", Тип("ПолеФормы"), ГруппаПрограммноСозданныеЭлементыФормы);		
		НовыйЭлемент.ПутьКДанным = "Объект.BKM_СтоимостьЧаса";
		НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
